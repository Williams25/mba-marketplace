...
import { parsePriceDividedHundred } from "@/utils/currencyUtils";
...
export const ProductCard = ({
  ...
  return (
    <Link to={DEFAULT_ROUTES.PRIVATE.PRODUCT_DETAILS.replace(":id", id)}>
      <Card className="w-80">
        <CardHeader className="p-2 relative">
          ...
          <CardTitle className="flex justify-between items-end px-4 pt-3 font-semibold text-lg text-gray-800">
            ...
            <span className="flex gap-3">
              <small>R$</small>{" "}
              {currencyMask(parsePriceDividedHundred(priceInCents))}
            </span>
          </CardTitle>
        </CardHeader>
        <CardContent>
